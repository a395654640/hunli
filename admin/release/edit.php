...
if($type == 1){
    $id=$_GET['id'];
 $sql="select * from releasee where id = $id";
 $resule=mysql_query($sql);
 $row=mysql_fetch_array($resule);
    require_once('./audit.html');
}else if($type == 2){
 $id=$_GET['id'];
    $sql="delete from releasee where id=$id";
    $rc=mysql_query($sql);
    if($rc){
        echo "<script type='text/javascript'>alert('需求发布删除成功！');history.go(-1);</script>";
        exit;
    }
}else if($type == 3){
    $title=$_GET['title'];
    $query="select count(*) from releasee where title like '%{$title}%'";
//设置每页显示多少条
    $PageSize = 9;
...
//获取当前页数
    ...
    $sql="select * from releasee where title like '%{$title}%'  order by id desc limit ".($page-1)*$PageSize.",".$PageSize;//注意limit后边有一空格。
...
//翻页链接
   ...
   $audit=@$_GET['audit'];
    $id=$_GET['id'];
    if($audit){
        $sql="update releasee set audit = 1 where id = {$id}";
    }else{
        $sql="update releasee set audit = 0 where id = {$id}";
    }
    $row=mysql_query($sql);
    if($row){
