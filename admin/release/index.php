...
if($_GET['type'] != 'b'){
    $type=$_GET['type'];
    $query="select count(*) from releasee where type= '{$type}'";
//设置每页显示多少条
    $PageSize = 9;
...
//获取当前页数
    ...
    $sql="select * from releasee where type= '{$type}'  order by id desc limit ".($page-1)*$PageSize.",".$PageSize;//注意limit后边有一空格。
