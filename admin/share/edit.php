...
if($type == 1){
 ...
 $id=$_GET['id'];
    ...
    $title=$_GET['title'];
    $query="select count(*) from share where title like '%{$title}%'";
//设置每页显示多少条
    $PageSize = 9;
...
//获取当前页数
    ...
    $sql="select * from share where title like '%{$title}%'  order by id desc limit ".($page-1)*$PageSize.",".$PageSize;//注意limit后边有一空格。
...
//翻页链接
   ...
   $audit=@$_GET['audit'];
    $id=$_GET['id'];
    if($audit){
        $sql="update share set audit = 1 where id = {$id}";
    }else{
        $sql="update share set audit = 0 where id = {$id}";
    }
    $row=mysql_query($sql);
    if($row){
