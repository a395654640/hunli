...
$type=$_GET['type'];
if($type == 1 ){
    $sql="select * from shareclass where sid = 0";//查询所有一级分类
    $resule=mysql_query($sql);
    require_once('./add.html');
}elseif($type == 2){
 $id=@$_POST['id'];
    if($id){//id存在时更新数据  不存在时写入数据
        $sql="update shareclass set name= '{$_POST['name']}',sid = '{$_POST['sid']}'  where id = {$_POST['id']} ";
    }else{
        $sql="insert into shareclass  (name,sid) VALUE ('{$_POST['name']}','{$_POST['sid']}') ";
    }
    ...
    $id=$_GET['id'];//查询id对应数据
    $sql="select * from shareclass where sid = 0";
    $resule=mysql_query($sql);
    $sqlc="select * from shareclass where id = $id";
    $resulec=mysql_query($sqlc);
    $class=mysql_fetch_array($resulec);
    require_once('./add.html');
}elseif($type == 4){
    $id=$_GET['id'];
    $sql="delete from shareclass where id=$id";
    $rc=mysql_query($sql);
    if($rc){
