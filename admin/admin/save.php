...
if($_POST['id']){
//id存在的话进行数据更新
    $sql="update admin set tel= '{$_POST['tel']}'  where id = {$_POST['id']} ";
    if($_POST['pass']){
        if($_POST['pass'] === $_POST['word'] ){
            $pass=sha1(md5(md5($_POST['pass'])));
            $sql="update admin set password= '{$pass}' , tel= '{$_POST['tel']}' where id = {$_POST['id']} ";
        }else{
...
//id不存在进行数据写入
    ...
    $csql="select * from admin where tel = '{$_POST['tel']}'";
    $pass=sha1(md5(md5($_POST['pass'])));
    $time=time();
    $cjg=mysql_query($csql);
    $hui=mysql_fetch_array($cjg);
    if(!$hui){
       $sql="insert into admin  (username,tel,password,type,time) VALUE ('{$_POST['name']}','{$_POST['tel']}','{$pass}','1','{$time}') ";
        $jieguo=mysql_query($sql);
       // echo $sql;
        if($jieguo){
            echo "<script type='text/javascript'>alert('管理员添加成功！');history.go(-1);</script>";
            exit();
        }else{
            echo "<script type='text/javascript'>alert('管理员添加失败！');history.go(-1);</script>";
            exit();
        }
    }else{
